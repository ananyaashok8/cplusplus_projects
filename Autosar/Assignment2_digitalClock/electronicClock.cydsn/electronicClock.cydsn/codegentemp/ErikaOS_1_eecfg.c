 ...
 **************************************************************************/
    ...
    #define TASK_4_STACK_SIZE 2048/4 // size = 2048 bytes
    int EE_cortex_mx_stack_4[TASK_4_STACK_SIZE];	/* Task 4 (tsk_control) */
    #endif
    #if ((1 == 2) && (EE_MAX_TASK > 4))
    #define TASK_5_STACK_SIZE 400/4 // size = 400 bytes
 ...
 **************************************************************************/
    ...
    const EE_oo_counter_object_ROM_type   EE_oo_counter_object_ROM[EE_COUNTER_OBJECTS_ROM_SIZE] = {
        #if EE_MAX_ALARM > 0
        {0, alarm_50ms, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 1
        ,{0, alarm_250ms, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 2
        ,{0, alarm_1min, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 3
        ,{0, alarm_250msEv, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 4
        ,{0, Alarm_5, EE_ALARM }
        #endif
